...
server <- function(input, output, session) {
  ...
  output$heatmap_real <- renderPlotly({
    ...
    plot_ly(data = heatmap_data, x = ~Var1, y = ~Var2, z = ~value, type = "heatmap", colors = "RdBu", zmin=-1, zmax=1, text = ~sprintf("%.2f", value), hoverinfo = "text") %>%
      layout(
        title = "Correlation Heatmap of Real Data",
        xaxis = list(tickangle = 45),
        yaxis = list(tickangle = -45),
        margin = list(l = 25, r = 10, b = 75, t = 75),
        width = 550,      # Further reduced width to 550
        height = 550      # Further reduced height to 550
      )
  })
  ...
  output$heatmap_virtual <- renderPlotly({
    ...
    plot_ly(data = heatmap_data, x = ~Var1, y = ~Var2, z = ~value, type = "heatmap", colors = "RdBu", zmin=-1, zmax=1, text = ~sprintf("%.2f", value), hoverinfo = "text") %>%
      layout(
        title = "Correlation Heatmap of Virtual Data",
        xaxis = list(tickangle = 45),
        yaxis = list(tickangle = -45),
        margin = list(l = 40, r = 10, b = 80, t = 80), # Adjusted margins for better fit
        width = 500,      # Adjusted width to 500
        height = 500      # Adjusted height to 500
      )
  })
  
  # Scatter plots comparison for Real Dataset
  output$pairplot_real <- renderPlot({
    req(realData())
    data_subset <- realData()
    numeric_vars <- sapply(data_subset, is.numeric)
    ggpairs(data_subset[, numeric_vars, drop = FALSE], title = "Scatter plots for Real Data", rescale = TRUE)  # Added rescale parameter
  })
  
  # Scatter plots comparison for Virtual Dataset
  output$pairplot_virtual <- renderPlot({
    req(virtualData())
    data_subset <- virtualData()
    numeric_vars <- sapply(data_subset, is.numeric)
    ggpairs(data_subset[, numeric_vars, drop = FALSE], title = "Scatter plots for Virtual Data", rescale = TRUE) # Added rescale parameter
  })
  ...
  output$downloadReport <- downloadHandler(
    filename = function() {
      paste("Data_Comparison_Report_", Sys.Date(), ".pdf", sep = "")
    },
    content = function(file) {
      # Open a PDF device
      pdf(file, width = 11, height = 8)
      
      # Histogram for virtual data
      hist(q.virtual, breaks = 40, probability = TRUE, ylim = c(0, 0.14), main = "Histogram of Q for Virtual Data")
      curve(dchisq(x, df = p), from = 0, to = max.q.virtual, lwd = 2, col = "blue", add = TRUE)
      
      # New page for the next plot
      dev.new()
      
      # Histogram for real data
      hist(q.real, breaks = 40, probability = TRUE, ylim = c(0, 0.14), main = "Histogram of Q for Real Data")
      curve(dchisq(x, df = p), from = 0, to = max.q.real, lwd = 2, col = "red", add = TRUE)
      ...
      # QQ Plot
      qqplot(q.virtual, q.real, main = "QQ Plot of Virtual vs. Real Data")
      abline(a = 0, b = 1, lwd = 2)
  ...
  output$bootstrapPlot <- renderPlot({
    ...
    d.virtual <- density(plotData$y.virtual)
    d.real <- density(plotData$y.real)
    plot(d.virtual, ylim = plotData$ylim.range, lwd = 2, col = "#1E6A9C", xlab = paste("Variable:", input$selected_var), main = "Comparison: Real vs. Virtual Cohort with 50 Bootstrap samples")
    lines(d.real, lwd = 2, col = "red")
    for(b in 1:50) {
  ...
  output$ggplot <- renderPlotly({
    ...
    p <- ggplot(dat, aes(x = x.boot)) + 
      ...
      geom_line(aes(y = y.boot.05, color = "Bootstrap 95% CI"), linetype = "solid") + 
    ...
    plotly_graph <- layout(plotly_graph, width = 800, height = 600)
  ...
  output$report <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!is.null(plotData)) {
        ...
        d.real <- density(plotData$y.real)
        plot(d.virtual, ylim = plotData$ylim.range, lwd = 2, col = "#1E6A9C", xlab = paste("Variable:", isolate(input$selected_var)),
             main = "Comparison: Real vs. Virtual Cohort with 50 Bootstrap samples")
        lines(d.real, lwd = 2, col = "red")
        for(b in 1:50) {
        ...
        p <- ggplot(dat, aes(x = x.boot)) + 
          geom_line(aes(y = y.boot.95, color = "Bootstrap 95%"), linetype = "solid") +
          geom_line(aes(y = y.boot.05, color = "Bootstrap 5%"), linetype = "solid") +
          geom_line(aes(x = x.real, y = d.real, color = "Real"), linetype = "solid") +
          geom_line(aes(x = x.virtual, y = d.virtual, color = "Virtual"), linetype = "solid") +
          ggtitle("Bootstrap validation") +
          xlab("x") + ylab("density") +
          ...
          labs(color = "Legend") +
          scale_color_manual(values = c("Bootstrap 95%" = "grey", 
                                        "Bootstrap 5%" = "grey",  
                                        "Real" = "red", 
                                        "Virtual" = "#1E6A9C"))
        print(p)
